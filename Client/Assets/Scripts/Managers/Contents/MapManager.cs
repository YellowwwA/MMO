...
public class MapManager
{
    ...
    bool[,] _collision;
    public bool CanGo(Vector3Int cellPos) //플레이어가 갈 수 있는 곳인지 없는 곳인지 체크
    {
    ...
    public void LoadMap(int mapId)
    {
        ...
        string mapName = "Map_" + mapId.ToString("000"); //맵 이름 포맷을 Map_000으로 설정 1,2,3이 되면 001,002,003으로 표기됨
        GameObject go = Managers.Resource.Instantiate($"Map/{mapName}");
        go.name = "Map";

        GameObject collision = Util.FindChild(go, "Tilemap_Collision", true);
        if (collision != null)
            collision.SetActive(false); //collision 파일은 게임 플레이 시 안보이게함 (맨첨에 가도 되는곳 안되는곳 탐색할때 쓰고 이후에는 안보이게 비활성화)

        CurrentGrid = go.GetComponent<Grid>();

        //Collision 관련 파일 가져오기
        TextAsset txt = Managers.Resource.Load<TextAsset>($"Map/{mapName}");
        StringReader reader = new StringReader(txt.text);

        //파일에서 정보 추출 하여 int로 변환시켜 데이터로 들고있기
